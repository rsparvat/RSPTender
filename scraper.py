...
def get(session,url,home):
    ...
    for wait in (0,2,5):
        ...
        try:
            r=session.get(url,timeout=(15,90),allow_redirects=True); r.raise_for_status()
            low=r.text.lower()
            if 'session has timed out' in low or 'unauthorised access' in low:
                session.get(home,timeout=(15,30)); r=session.get(url,timeout=(15,90)); r.raise_for_status()
            return r
...
def parse_detail(session,source,org,url,home):
    ...
    nit=exact(soup,'Tender Reference Number','Tender Ref. No.','Tender Ref.No')
    work=exact(soup,'Work Description','Tender Title','Title')
    loc=exact(soup,'Location') or exact(soup,'Bid Opening Place')
    pin=exact(soup,'Pincode','Pin Code')
    pub=exact(soup,'Published Date','Publish Date','Publication Date')
    end=exact(soup,'Bid Submission End Date','Bid Submission Closing Date','Submission End Date')
    ...
    tf=money(exact(soup,'Tender Fee in ₹','Tender Fee','Document Fee','Tender Document Fee'))
    pf=money(exact(soup,'Processing Fee in ₹','Processing Fee','Portal Fee'))
    emd=money(exact(soup,'EMD Amount in ₹','EMD Amount','EMD Fee','Earnest Money Deposit'))
    chain=exact(soup,'Organisation Chain','Organization Chain'); parts=[x.strip() for x in chain.split('||') if x.strip()]
    project=parts[1] if source=='COAL' and len(parts)>1 else ''
    organisation=parts[0] if parts else org
    district=''
    ...
    for a in soup.find_all('a',href=True):
        ...
        if 'corrigendum' in txt.lower() or 'corrigendum' in hl:
            corrs.append({'title':txt or 'Corrigendum','url':urljoin(PORTALS[source][1],a['href'])})
...
def scan(source,old):
    name,base,orgurl=PORTALS[source]; home=base+'/nicgep/app'; s=requests.Session(); s.headers.update({'User-Agent':UA,'Accept-Language':'en-IN,en;q=0.9'})
    found={}; orgs=org_links(s,base,orgurl,home,source)
    # Coal desktop logic prioritises NCL. Web run remains bounded to keep Actions reliable.
    if source=='COAL':
        priority=[x for x in orgs if 'northern coalfields limited' in x[0].lower()]; rest=[x for x in orgs if x not in priority]; orgs=priority+rest[:20]
    for oi,(org,u) in enumerate(orgs):
        try: links=tender_links(s,base,u,home)
        ...
        if source=='COAL' and oi>=20: break
    # retain previous cache so temporary NIC failures do not blank website
    for t in old:
...
def main():
    ...
    for src in ('MP','COAL'):
        try: rows.extend(scan(src,old))
            ...
            errors.append(f'{src}: {e}'); rows.extend([x for x in old if x.get('source')==src])
    payload={'generated_at':datetime.now(timezone.utc).isoformat(),'base_version':'2.6.1 AREA_PARSER_FIX','count':len(rows),'errors':errors,'tenders':rows}
